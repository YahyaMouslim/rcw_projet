...
export class SidebarComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    console.log(userid)
    if (userid)
    this.authService.getByID(userid).subscribe(res=>{
      const role =res.role
      console.log(role)
      console.log(MENU)
      if(role.toString()=='INSTRUCTOR')
       this.menuItems = MENU.filter((r)=>r.label=='Emplois du temps' || r.label=='Planification');
      else if(role.toString()=='USER')
        this.menuItems = MENU.filter((r)=>r.label=='Emplois du temps');
